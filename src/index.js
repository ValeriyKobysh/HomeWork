...
function findAllPSiblings(where) {
    ...
    while (child != null) {
        ...
        if (nextChild != null && nextChild.tagName === 'P') {
            array.push(child);
        }
...
function collectDOMStat(root) {
    let tagsObj = {},
        ...
        texts = 0;
    function stat(start) {
        let children = start.firstElementChild;
        while (children != null) {
            let tag = children.tagName,
                classNameList = children.classList;
            stat(children);
            (tag in tagsObj) ? tagsObj[tag] += 1 : tagsObj[tag] = 1;
            for (var i = 0; i < classNameList.length; i++) {
                (classNameList[i] in classesObj) ? classesObj[classNameList[i]] += 1 : classesObj[classNameList[i]] = 1;
            }
            texts++;
